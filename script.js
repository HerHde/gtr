$(document).ready(function(){
	...
	$("#opt11").click(function(){
		$("#opt1").html('Godlike! So einen Stealth-Kill hat Rodgau selten gesehen. Du murckst ihn ohne jeden Ton ab, steckst die Heugabel wieder in deine sehr geräumige GTA-Tasche und bekommst <b>666 Erfahrungspunkte</b>. Aber was ist mit dem Trecker?');
		$("#opt2").fadeIn();
	});
	...
	$("#opt21").click(function(){
		$("#opt2").html('Das Spektakel erinnert ein wenig an Cobra 11. Jeder in Rodgau City kriegt die Explosion mit, aber keine Sorge: Deine Nachbarn wissen die restlose Vernichtung des Eindringlings zu schätzen. <b>Dein Ruf in Rodgau hat sich um 100 Punkte verbessert</b> Außerdem feiert die Gemeinde ein Osterfeuer.');
		$("#trecker2 .bNext").fadeIn();
	});

	$("#opt22").click(function(){
		$("#opt2").html('Der Trecker gleitet sanft in die Tasche, ohne sie auch nur annähernd auszubeulen, doch als du weitergehen willst trifft es dich wie ein Lag: <b>Ihr seid zu schwer belastet.</b> Verdammte Oblivion-Engine. Aber wenn schon Oblivion, dann richtig: Du zündest den entpackten Trecker per Feuerball an und durchsuchst das Wrack, doch leider findest du nichts außer ein paar Löffeln, Gabeln und anderen Dingen, die die Spielatmosphäre verbessern sollen aber keinen besonderen Nutzen haben.');
		$("#trecker2 .bNext").fadeIn();
	});
